...
- **Watch 运动与系统运动冲突 → 共享消耗合并**：磨平运动被系统（Apple 健身）运动抢占结束后，回到磨平 App（scenePhase 回前台）会查 `[磨平开始, 现在]` 的 HealthKit 活动能量总和（含磨平 + 系统两段），合并回任务 `burnedCalories`——达标即结清、未达标更新进度并保留可继续。静默不弹窗（watchOS 不允许实时暂停/后台弹窗）。开始时刻用 UserDefaults 记（不改 SwiftData model）；纯决策 `WorkoutMergePlan`（`LevelItShared`）+ 单测 6 例。
